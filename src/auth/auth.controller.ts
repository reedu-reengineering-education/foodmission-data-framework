import { Controller, Get, Request, UnauthorizedException, UseGuards, Put, Body } from '@nestjs/common';
import {
  ApiTags,
  ApiOperation,
  ApiBearerAuth,
...
export class AuthController {
  ...
  async updatePreferences(@Request() req: any, @Body() preferences: any) {
    ...
    return await this.userProfileService.updatePreferences(user.sub, preferences);
  }
  ...
  async getTokenInfo(@Request() req: any) {
    ...
    return {
      ...
      given_name: user.given_name,
      family_name: user.family_name,
      roles: user.resource_access?.[process.env.KEYCLOAK_CLIENT_ID || 'foodmission-api']?.roles || [],
      exp: user.exp,
      iat: user.iat,
    };
  }
