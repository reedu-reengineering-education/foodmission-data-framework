...
describe('MissionProgressController', () => {
	...
	describe('getAll', () => {
		it('should call service.getAllMissionsByUserId and return result', async () => {
			const mockResult = [{ missionId: 'm1' }];
			(service.getAllMissionsByUserId as jest.Mock).mockResolvedValue(mockResult);
			const result = await controller.getAll('u1');
			expect(service.getAllMissionsByUserId).toHaveBeenCalledWith('u1');
