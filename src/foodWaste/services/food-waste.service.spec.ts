...
import { FoodWasteService, UNIT_TO_KG_CONVERSION, CARBON_ESTIMATES_BY_CATEGORY } from './food-waste.service';
...
describe('FoodWasteService', () => {
  ...
  describe('create', () => {
    ...
    it('should create a food waste entry successfully', async () => {
      ...
      const result = await service.create(TEST_CREATE_FOOD_WASTE_DTO as any, userId);

      expect(result).toBeDefined();
      expect(result.id).toBe(mockFoodWaste.id);
      expect(foodRepository.findById).toHaveBeenCalledWith(TEST_CREATE_FOOD_WASTE_DTO.foodId);
    ...
    it('should validate pantry item ownership when pantryItemId provided', async () => {
      ...
      expect(result).toBeDefined();
      expect(pantryItemRepository.findById).toHaveBeenCalledWith('pantry-item-1');
    });
    ...
    it('should throw error if pantry item belongs to another user', async () => {
      ...
      foodRepository.findById.mockResolvedValue(mockFood);
      pantryItemRepository.findById.mockResolvedValue(otherUserPantryItem as any);

      await expect(
        service.create(dtoWithPantryItem as any, userId),
      ).rejects.toThrow('Cannot create waste entry for pantry item that does not belong to you');
    });
  });
  ...
  describe('findAll', () => {
    it('should return paginated food waste entries', async () => {
      ...
      foodWasteRepository.findWithPagination.mockResolvedValue(mockPaginatedResult);

      const result = await service.findAll('user-1', TEST_QUERY_FOOD_WASTE_DTO as any);
  ...
  describe('findOne', () => {
    ...
    it('should throw ForbiddenException if entry belongs to another user', async () => {
      ...
      await expect(
        service.findOne('food-waste-1', 'user-1'),
  ...
  describe('update', () => {
    ...
    it('should throw NotFoundException if entry does not exist', async () => {
      ...
      await expect(
        service.update('non-existent', TEST_UPDATE_FOOD_WASTE_DTO as any, 'user-1'),
      ).rejects.toThrow(NotFoundException);
    });
    ...
    it('should throw ForbiddenException if entry belongs to another user', async () => {
      ...
      await expect(
        service.update('food-waste-1', TEST_UPDATE_FOOD_WASTE_DTO as any, 'user-1'),
      ).rejects.toThrow(ForbiddenException);
    });
  ...
  describe('remove', () => {
    ...
    it('should throw NotFoundException if entry does not exist', async () => {
      ...
      await expect(
        service.remove('non-existent', 'user-1'),
      ).rejects.toThrow(NotFoundException);
    });
    ...
    it('should throw ForbiddenException if entry belongs to another user', async () => {
      ...
      await expect(
        service.remove('food-waste-1', 'user-1'),
      ).rejects.toThrow(ForbiddenException);
    });
  });
  ...
  describe('getTrends', () => {
    it('should return food waste trends', async () => {
      const mockTrends = [
        { date: new Date('2026-02-13'), totalWaste: 1.5, totalCost: 5.99, totalCarbon: 1.25, count: 1 },
        { date: new Date('2026-02-14'), totalWaste: 0.5, totalCost: 2.5, totalCarbon: 0.75, count: 1 },
      ];
    ...
    it('should support different intervals', async () => {
      const mockTrends = [
        { date: new Date('2026-02-01'), totalWaste: 10, totalCost: 50, totalCarbon: 15, count: 5 },
      ];
  ...
  describe('calculateCarbonFootprint', () => {
    it('should calculate carbon footprint for food', async () => {
      ...
      const result = await service.calculateCarbonFootprint('food-1', 1.5, Unit.KG);
    ...
    it('should return default estimate when food not found', async () => {
      ...
      const result = await service.calculateCarbonFootprint('non-existent', 1.0, Unit.KG);

      const expectedDefault = 1.0 * UNIT_TO_KG_CONVERSION[Unit.KG] * CARBON_ESTIMATES_BY_CATEGORY.default;
    ...
    it('should convert units correctly', async () => {
      ...
      const resultKg = await service.calculateCarbonFootprint('food-1', 1, Unit.KG);
      const resultG = await service.calculateCarbonFootprint('food-1', 1000, Unit.G);
  ...
  describe('batchCreateFromExpired', () => {
    it('should batch create waste entries from expired pantry items', async () => {
      const batchDto = {
        items: [
          { pantryItemId: 'pantry-item-1', notes: 'Test notes' },
        ],
      };
      ...
      const result = await service.batchCreateFromExpired(batchDto as any, 'user-1');
